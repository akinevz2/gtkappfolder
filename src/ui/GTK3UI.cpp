...
GTK3UI::GTK3UI() 
    : window(nullptr), main_vbox(nullptr), path_entry(nullptr),
...
void GTK3UI::initialize(int argc, char* argv[]) {
    gtk_init(&argc, &argv);
}

void GTK3UI::create_window(const std::string& title, int width, int height) {
    window = gtk_window_new(GTK_WINDOW_TOPLEVEL);
    gtk_window_set_title(GTK_WINDOW(window), title.c_str());
    gtk_window_set_default_size(GTK_WINDOW(window), width, height);
    gtk_container_set_border_width(GTK_CONTAINER(window), 10);
    g_signal_connect(window, "destroy", G_CALLBACK(on_destroy_cb), this);
    
    main_vbox = gtk_box_new(GTK_ORIENTATION_VERTICAL, 5);
    gtk_container_add(GTK_CONTAINER(window), main_vbox);
}

void GTK3UI::show_and_run() {
...
void GTK3UI::create_path_bar(const std::string& initial_path) {
    ...
    gtk_entry_set_text(GTK_ENTRY(path_entry), initial_path.c_str());
    ...
    g_signal_connect(path_entry, "button-press-event", G_CALLBACK(on_path_button_press_cb), this);
...
void GTK3UI::create_content_area() {
...
void GTK3UI::update_path_completion(const std::string& current_text) {
...
std::string GTK3UI::get_path_entry_text() const {
    return gtk_entry_get_text(GTK_ENTRY(path_entry));
}

void GTK3UI::set_path_entry_text(const std::string& text) {
    gtk_entry_set_text(GTK_ENTRY(path_entry), text.c_str());
}

void GTK3UI::clear_content() {
...
void GTK3UI::add_appimage_tile(const std::string& path, const std::string& filename,
...
void GTK3UI::show_empty_message(const std::string& message) {
...
void GTK3UI::refresh_content() {
    gtk_widget_show_all(flow_box);
}

void GTK3UI::set_path_changed_callback(PathChangedCallback callback) {
    on_path_changed = callback;
}

void GTK3UI::set_refresh_callback(RefreshCallback callback) {
    on_refresh = callback;
}

void GTK3UI::set_appimage_click_callback(AppImageClickCallback callback) {
    on_appimage_click = callback;
}

// Static callbacks
void GTK3UI::on_destroy_cb(GtkWidget* widget, gpointer user_data) {
    GTK3UI* ui = static_cast<GTK3UI*>(user_data);
    ui->quit();
}

void GTK3UI::on_path_activate_cb(GtkEntry* entry, gpointer user_data) {
    GTK3UI* ui = static_cast<GTK3UI*>(user_data);
...
void GTK3UI::on_refresh_clicked_cb(GtkButton* button, gpointer user_data) {
    GTK3UI* ui = static_cast<GTK3UI*>(user_data);
    if (ui->on_refresh) {
        ui->on_refresh();
    }
...
void GTK3UI::on_appimage_clicked_cb(GtkButton* button, gpointer user_data) {
    GTK3UI* ui = static_cast<GTK3UI*>(user_data);
...
void GTK3UI::on_path_entry_changed_cb(GtkEditable* editable, gpointer user_data) {
    GTK3UI* ui = static_cast<GTK3UI*>(user_data);
...
gboolean GTK3UI::on_path_button_press_cb(GtkWidget* widget, GdkEventButton* event, gpointer user_data) {
    GTK3UI* ui = static_cast<GTK3UI*>(user_data);
...
gboolean GTK3UI::on_completion_match_cb(GtkEntryCompletion* completion, 
...
gboolean GTK3UI::on_completion_match_selected_cb(GtkEntryCompletion* completion,
                                                 GtkTreeModel* model,
                                                 GtkTreeIter* iter,
                                                 gpointer user_data) {
    GTK3UI* ui = static_cast<GTK3UI*>(user_data);
